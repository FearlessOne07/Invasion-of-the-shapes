...
BulletManager::BulletManager(std::shared_ptr<AssetManager> assets, std::shared_ptr<Camera2D> camera)
...
void BulletManager::CheckBullets() {
  ...
  auto it =
      std::remove_if(_bullets.begin(), _bullets.end(), [](Bullet &b) { return !b.isActive(); }  //
      );

  _bullets.erase(it, _bullets.end());
}

void BulletManager::SpawnBullet(const Vector2 &playerPos, const Vector2 &target,
                                Bullet::BulletTag tag, float speed) {
  Vector2 direction = Vector2Subtract(target, playerPos);
  direction = Vector2Normalize(direction);
  _bullets.emplace_back(
      Bullet(playerPos, direction, _assets->GetTexture("bullet"), _camera, tag, speed));
