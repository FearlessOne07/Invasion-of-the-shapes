...
GameOverScene::GameOverScene(std::shared_ptr<Game> game, std::shared_ptr<AssetManager> assets)
    : Scene(game, assets) {
...
void GameOverScene::Render() {
  ...
  if (_inputTimer >= _inputLimit) {
    DrawTextEx(*_font, _playText, {_playTextX, 600}, 30, 1,
               {255, 0, 0, static_cast<unsigned char>(_opacity)});
  }
}
