...
GameScene::GameScene(std::shared_ptr<Game> game, std::shared_ptr<AssetManager> assets)
    : Scene(game, assets),
      ...
      _player(std::make_shared<Player>(_playerStart, _playerColor, _assets, _bulMan, _camera)) {
...
void GameScene::Render() {
  ...
  DrawTextEx(*(_assets->GameFont()), TextFormat("Score:%i", _player->GetScore()), {10, 10}, 24, 1,
             WHITE);
  DrawText(TextFormat("Camera Target:(%f, %f)", _camera->target.x, _camera->target.y), 0, 50, 24,
           WHITE);
  BeginMode2D(*_camera);
  _enemMan.Render();
  _player->Render();
...
void GameScene::UpdateCamera(float dt) {
  ...
  _camera->target =
      Vector2MoveTowards(_camera->target, _player->GetPos(), static_cast<int>(maxMove * dt));
